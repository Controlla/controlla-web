<template>
  <section class="choose">
    <article
      ...
      class="choose__content"
    >
      <header class="choose__content__header">
        <h2
          v-motion
          :="sm ? animations.noop : animations.popVisibleOnce"
          class="choose__content__header__title"
        >
          {{ $t('choose.title.1') }}
          <span class="choose__content__header__title--blue">
            {{ $t('choose.title.2') }}
          </span>
          {{ $t('choose.title.3') }}
        </h2>
        <p
          v-motion
          :="sm ? animations.noop : animations.popVisibleOnce"
          class="choose__content__header__text"
        >
          {{ $t('choose.text') }}
        </p>
      </header>
      <footer class="choose__content__footer">
        <ul
          v-motion
          :="sm ? animations.noop : animations.slideVisibleOnceBottom"
          class="choose__content__footer__list"
          role="list"
        >
          <li
            v-for="card in cards"
            :key="card.title"
            class="choose__content__footer__list__item"
            role="listitem"
          >
            <v-card>
              <template #icon>
                <v-image
                  ...
                  class="choose__content__footer__list__item__image"
                />
              </template>
              <template #header>
                <h3 class="choose__content__footer__list__item__title">
                  {{ $t(card.title) }}
                </h3>
              </template>
              <template #body>
                <p
                  class="choose__content__footer__list__item__text"
                  v-html="getFormatedText(card.text)"
                />
              </template>
            </v-card>
          </li>
...
<script lang="ts">
import hugeImageUrl from '@/assets/images/icons/huge.webp'
import planeImageUrl from '@/assets/images/icons/plane.webp'
import targetImageUrl from '@/assets/images/icons/target.webp'
...
export default defineComponent({
  ...
  setup() {
    ...
    const getFormatedText = (texts: Array<{ type: string; text: string }>) => {
      let htmlText = ''

      texts.forEach((text) => {
        text.type === 'normal'
          ? (htmlText += t(text.text))
          : (htmlText += `<span> ${t(text.text)} </span>`)
      })
